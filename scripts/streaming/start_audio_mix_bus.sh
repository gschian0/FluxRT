...
MUSIC_MIX_VOLUME="${MUSIC_MIX_VOLUME:-0.55}"
TTS_MIX_VOLUME="${TTS_MIX_VOLUME:-0.95}"
...
while true; do
  ffmpeg -hide_banner -loglevel info \
    ...
    -thread_queue_size 16384 -use_wallclock_as_timestamps 1 -i "${MUSIC_INPUT_URL}" \
    -thread_queue_size 16384 -use_wallclock_as_timestamps 1 -i "${TTS_INPUT_URL}" \
    -filter_complex "[1:a]aresample=async=1:min_hard_comp=0.100:first_pts=0,asetpts=N/SR/TB,volume=${MUSIC_MIX_VOLUME}[music];[2:a]aresample=async=1:min_hard_comp=0.100:first_pts=0,asetpts=N/SR/TB,volume=${TTS_MIX_VOLUME}[ttsv];[ttsv]asplit=2[ttsd][ttsm];[music][ttsd]sidechaincompress=threshold=0.02:ratio=5:attack=80:release=600[ducked];[ducked][ttsm]amix=inputs=2:duration=longest:dropout_transition=0:normalize=0,alimiter=limit=0.95,aresample=async=1:min_hard_comp=0.100:first_pts=0[aout]" \
    -map "[aout]" \
    -c:a aac -b:a "${AUDIO_BITRATE}" -ar 48000 -ac 2 \
    -f mpegts "${MIX_OUTPUT_URL}" || true
  sleep 1
