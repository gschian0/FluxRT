...
if [[ "$STARTUP_BARS_SECONDS" =~ ^[0-9]+$ ]] && [[ "$STARTUP_BARS_SECONDS" -gt 0 ]]; then
  echo "Starting color bars preroll for ${STARTUP_BARS_SECONDS}s..."
  run_bars_segment "$STARTUP_BARS_SECONDS"
  if [[ "$WAIT_FOR_VIDEO_READY" == "1" ]]; then
    extensions=0
    while ! video_input_ready; do
      if [[ "$extensions" -ge "$MAX_BARS_EXTENSIONS" ]]; then
        echo "Video input still not ready after ${extensions} extensions; forcing live handoff."
        break
      fi
      echo "Video input not ready yet; extending bars by ${STARTUP_BARS_EXTEND_SECONDS}s..."
      run_bars_segment "$STARTUP_BARS_EXTEND_SECONDS"
      extensions=$((extensions + 1))
    done
  fi
  echo "Switching from bars to live feed."
fi
